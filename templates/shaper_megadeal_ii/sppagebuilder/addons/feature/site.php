...
class SppagebuilderAddonFeature extends SppagebuilderAddons {
	public function render() {
		...
		$icon_library = (isset($this->addon->settings->icon_library) && $this->addon->settings->icon_library) ? $this->addon->settings->icon_library : 'fontawesome';
		$icon_name_mdeal = (isset($this->addon->settings->icon_name_mdeal) && $this->addon->settings->icon_name_mdeal) ? $this->addon->settings->icon_name_mdeal : '';
		...
		if($feature_type == 'icon') {
			if($icon_name) {
				...
				$media  .= '<span style="display:inline-block;">';

				$media  .= '<span class="sppb-icon-container">';
				if ($icon_library='meadeal' && $icon_name_mdeal) {
					$media  .= '<i class="md ' . $icon_name_mdeal . '" ></i>';
				}else{
		...
		if($title) {
			...
			$feature_title .= '<'.$heading_selector.' class="sppb-feature-box-title'. $heading_class .'">' . $title . '</'.$heading_selector.'>';
