...
if (($product->product_in_stock - $product->product_ordered) < 1) {
	...
	<div class="availability">
		<p class="vm-count-down-text">
			<?php echo JText::_('COM_VIRTUEMART_TIME_LEFT_BUY'); ?>
		</p>
		<div class="sp-vm-countdown" data-countdown="<?php echo JHtml::_('date', $product->product_available_date, vmText::_('Y-m-d')); ?>"></div>
		<?php if($tpl_params->get('vm_show_avl_date', false)){ ?>
		<div class="vm-product-availability-date">
			<?php echo vmText::_('COM_VIRTUEMART_PRODUCT_AVAILABLE_DATE') .': '. JHtml::_('date', $product->product_available_date, vmText::_('DATE_FORMAT_LC4')); ?>
		</div> <!-- /.vm-product-availability-date -->
		<?php } ?>
	</div>
<?php } ?>